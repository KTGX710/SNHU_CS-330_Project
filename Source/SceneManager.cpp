...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("lightSource[0].ambientColor", 0.0126f, 0.0124f, 0.0108f);
	m_pShaderManager->setVec3Value("lightSource[0].diffuseColor", 0.01f, 0.01f, 0.01f);
	m_pShaderManager->setVec3Value("lightSource[0].specularColor", 0.001f, 0.001f, 0.001f);
	m_pShaderManager->setFloatValue("lightSource[0].focalStrength", 16.0f);
	m_pShaderManager->setFloatValue("lightSource[0].specularIntensity", 0.01f);

	// Left
	m_pShaderManager->setVec3Value("lightSources[1].position", -10.0f, 10.0f, -10.0f);
	m_pShaderManager->setVec3Value("lightSources[1].ambientColor", 0.0126f, 0.0124f, 0.0108f);
	m_pShaderManager->setVec3Value("lightSources[1].diffuseColor", 0.01f, 0.01f, 0.01f);
	m_pShaderManager->setVec3Value("lightSources[1].specularColor", 0.001f, 0.001f, 0.001f);
	...
	// Center
	m_pShaderManager->setVec3Value("lightSource[2].position", 0.0f, 15.0f, -10.0f);
	m_pShaderManager->setVec3Value("lightSources[2].ambientColor", 0.0252f, 0.0249f, 0.0217f);
	m_pShaderManager->setVec3Value("lightSources[2].diffuseColor", 0.0252f, 0.0249f, 0.0217f);
	m_pShaderManager->setVec3Value("lightSources[2].specularColor", 0.0252f, 0.0249f, 0.0217f);
	...
	int nextUnusedLight = 3;
	// Loop to disable lights
	if (nextUnusedLight < 4) for (unsigned int index = nextUnusedLight; index < 4; ++index) {
		m_pShaderManager->setVec3Value("lightSource[" + std::to_string(index) + "].position", 0.0f, 0.0f, 0.0f);
		m_pShaderManager->setVec3Value("lightSources[" + std::to_string(index) + "].ambientColor", 0.0f, 0.0f, 0.0f);
		m_pShaderManager->setVec3Value("lightSources[" + std::to_string(index) + "].diffuseColor", 0.0f, 0.0f, 0.0f);
		m_pShaderManager->setVec3Value("lightSources[" + std::to_string(index) + "].specularColor", 0.0f, 0.0f, 0.0f);
		m_pShaderManager->setFloatValue("lightSources[" + std::to_string(index) + "].focalStrength", 1.0f);
		m_pShaderManager->setFloatValue("lightSources[" + std::to_string(index) + "].specularIntensity", 0.0f);
	}
...
void SceneManager::PrepareScene()
{
	...
	/* Comment out unused shapes to save memory */
 ...
 ***********************************************************/
std::vector<GLfloat> SceneManager::getRGB(int u_HexValue, GLfloat u_alpha)
{
	GLfloat r = GLfloat((u_HexValue >> 16) & 0xFF) / 255.0f;
	GLfloat g = GLfloat((u_HexValue >> 8) & 0xFF) / 255.0f;
	GLfloat b = GLfloat(u_HexValue & 0xFF) / 255.0f;
 ...
 *  getHex()
 *
 * This method is a helper function to convert a Vec3
 * RGB value set into a hexadecimal value to be passed
 * to getRGB() and drawShape().
 ***********************************************************/
int SceneManager::getHex(int r, int g, int b) {
	int rHex = (r & 0xFF) << 16;
	int gHex = (g & 0xFF) << 8;
	int bHex = (b & 0xFF);
...
)
{
	...
	GLfloat XrotationDegrees = (xRot);
	GLfloat YrotationDegrees = (yRot);
	GLfloat ZrotationDegrees = (zRot);
	...
	if (!tex.empty()) {
		...
		std::vector<GLfloat> rgb = getRGB(colorHex, colorAlpha);
	...
	/* Uncomment loaded shapes to be used */
	/* !!! Be sure to uncomment shapes in PrepareScene() as well, so that meshes are loaded into memory !!! */
	if (shape == "plane") m_basicMeshes->DrawPlaneMesh();
	else if (shape == "box") m_basicMeshes->DrawBoxMesh();
	else if (shape == "cylinder") m_basicMeshes->DrawCylinderMesh();
	else if (shape == "cylinderT") m_basicMeshes->DrawCylinderMesh(true, true, false);
	else if (shape == "cylinderS") m_basicMeshes->DrawCylinderMesh(false, false, true);
	// else if (shape == "tcylinder") m_basicMeshes->DrawTaperedCylinderMesh();
	// else if (shape == "cone") m_basicMeshes->DrawConeMesh();
	else if (shape == "sphere") m_basicMeshes->DrawSphereMesh();
	else if (shape == "hsphere") m_basicMeshes->DrawHalfSphereMesh();
	// else if (shape == "prism") m_basicMeshes->DrawPrismMesh();
	// else if (shape == "pyramid3") m_basicMeshes->DrawPyramid3Mesh();
	// else if (shape == "pyramid4") m_basicMeshes->DrawPyramid4Mesh();
	// else if (shape == "torus") m_basicMeshes->DrawTorusMesh();
	// else if (shape == "htorus") m_basicMeshes->DrawHalfTorusMesh();
...
void SceneManager::RenderScene()
{
	...
	// Caviar
	/***
	* Create a stacked conical shape for caviar
	***/
	for (unsigned int i = 7; i > 0; --i) // Layers, starting at top
	{
		// Number of elements in layer
		unsigned int k = 4 * i;
		for (unsigned int j = 0; j < k; ++j) // Object generation loop
		{
			GLfloat posAngle = (360.0f / k) * j;
			GLfloat posZ = sin(glm::radians(posAngle));
			GLfloat posX = cos(glm::radians(posAngle));
	...
	for (unsigned int i = 0; i < 8; ++i) {
		GLfloat posAngle = 180.0f + (i * 360 / 20);
		GLfloat posZ = sin(glm::radians(posAngle));
		GLfloat posX = cos(glm::radians(posAngle));
	...
	// Olives
	for (unsigned int i = 0; i < 6; ++i) // Create flowered pattern
	{
		GLfloat posAngle = (i * 360.0f / 6.0f);
		GLfloat posZ = sin(glm::radians(posAngle));
		GLfloat posX = cos(glm::radians(posAngle));
